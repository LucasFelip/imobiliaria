...
public class ImovelRepositoryQueryImpl implements ImovelRepositoryQuery {
    @PersistenceContext
    ...
    public Page<Imovel> filtrar(ImovelFilter filtro, Pageable pageable) {
        ...
        // 3. adiciona as restrições (os predicados) que serão passadas na clausula
        // where
    ...
    private Predicate[] criaRestricoes(ImovelFilter filtro, CriteriaBuilder cBuilder, Root<Imovel> produtoRoot) {
        ...
        if (!StringUtils.isEmpty(filtro.getTipoImovel())) {
            ...
            predicates.add(cBuilder.like(cBuilder.lower(produtoRoot.get("tipoImovel")),
                    "%" + filtro.getTipoImovel().toLowerCase() + "%"));
        }
